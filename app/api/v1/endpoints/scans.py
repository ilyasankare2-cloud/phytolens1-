...
def get_model_information(current_user: User = Depends(deps.get_current_user)):
    """
...
def read_user_scans(
    ...
    """
    scans = db.query(ScanModel).filter(ScanModel.user_id == current_user.id).offset(skip).limit(limit).all()
    return scans
